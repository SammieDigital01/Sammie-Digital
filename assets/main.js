...
/* ---- Google-review carousel (Home page testimonials) ---- */
document.addEventListener('DOMContentLoaded', () => {
  const slider = document.querySelector('.greview-slider');
  if (!slider) return;
  const cards = slider.querySelectorAll('.greview-card');
  const dotsWrap = document.querySelector('.greview-dots');
  const prevBtn = document.querySelector('.greview-prev');
  const nextBtn = document.querySelector('.greview-next');
  if (!cards.length) return;

  const cardWidth = () => cards[0].offsetWidth + 24; // gap
  const visibleCount = () => Math.max(1, Math.round(slider.clientWidth / cardWidth()));
  ...
  const buildDots = () => {
    ...
    for (let i = 0; i < pageCount(); i++) {
      ...
      d.addEventListener('click', () => {
        slider.scrollTo({ left: i * visibleCount() * cardWidth(), behavior: 'smooth' });
      });
  ...
  const updateDots = () => {
    const dots = dotsWrap.querySelectorAll('.greview-dot');
    const page = Math.round(slider.scrollLeft / (visibleCount() * cardWidth()));
    dots.forEach((d, i) => d.classList.toggle('active', i === page));
  };

  slider.addEventListener('scroll', () => {
    clearTimeout(slider._t);
    slider._t = setTimeout(updateDots, 80);
  }, { passive: true });

  prevBtn && prevBtn.addEventListener('click', () => slider.scrollBy({ left: -visibleCount() * cardWidth(), behavior: 'smooth' }));
  nextBtn && nextBtn.addEventListener('click', () => slider.scrollBy({ left: visibleCount() * cardWidth(), behavior: 'smooth' }));
